...
> 记录日期：2026-09-10 ｜ 相关决策见 [../docs/tech.md](../docs/tech.md) §5.3.1 / §9.1
...
> ⚠️ **对已存在的数据库，这个脚本不会重跑。**
> 因此**扩展与检索配置的权威来源是 EF 迁移**（见 [../docs/backend.md](../docs/backend.md) §3.8）。
...
> 表现为「数据看起来丢了」（实际还在卷里）。
>
> **本步骤尚未执行** —— 需要你确认后再切换（见 [../docs/suggestion.md](../docs/suggestion.md) T11 的选项）。

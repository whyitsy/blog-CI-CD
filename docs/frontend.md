...
> 编辑页统一用 `getPostDetailReadonly`：既避免污染浏览量，又由后端做归属校验
> （非本人文章返回 404），因此作者无法通过改 URL 编辑他人文章。

> `[待确认]` 分类/标签写接口目前仅 Admin 可用（T6 决策），因此作者在编辑器里
> 「快速新建分类/标签」会被后端 403。处理选项见 [suggestion.md](./suggestion.md) T13。
...
| # | 路由 | 状态 |
|---|---|---|
| 26 | `/:pathMatch(.*)*` → `NotFoundView` | `[已实现]`（替换了原来的 `redirect: '/'`，消除软 404） |

> `[已实现]` 已改为渲染 `NotFoundView`：展示 404 与出错路径，并提供返回首页/上一页。
> 生产环境还需让服务器对未知路径返回真实 404 状态码（见 [tech.md](./tech.md) §6.3 第 1 步）。

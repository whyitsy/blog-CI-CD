...
| 项 | 值 |
|---|---|
| 数据库 | PostgreSQL **18.6**（需 zhparser 扩展，见 §7） |
| 库名（开发） | `blog_stage2` |
...
| 来源 | 覆盖场景 | 内容 |
|---|---|---|
| `deploy/postgres-init/01-zhparser.sql` | 全新数据库（entrypoint 只在数据目录为空时执行） | `CREATE EXTENSION IF NOT EXISTS zhparser` + 建 `chinese` 配置 |
| EF 迁移 `20260910205225` | **已有数据库**（entrypoint 不会重跑） | 同样的语句 + GIN 索引 |
...
**两处都要有的原因**：`postgres` 官方镜像的 entrypoint **只在数据目录为空时**执行
`/docker-entrypoint-initdb.d/` 下的脚本。对已存在的库它不会重跑，
所以扩展的权威来源必须是迁移（迁移每次启动都会应用）。

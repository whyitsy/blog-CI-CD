...
（**注**：下文两个登录入口已于后续评审中**合并为同一个 `/login`**，见本节末尾的修订说明。）
...
> **修订（已合并为单一登录入口）**：早期结论是「需要两套入口」，现已推翻，理由如下：
>
> 1. **两个登录页登录的都是 `User` 账号**，`Author` 实体里没有 `PasswordHash`/`Role`
>    （`Blog.Domain/Entities/Author.cs` 只有 `Name/Email/Avatar/Bio`），**根本无法登录**。
>    「作者登录」= 登录一个 `role=Author` 的 **User 账号**，与署名实体 `Author` 撞名，
>    是反复产生误会的根源。
> 2. **页面分离不提供任何安全性**。真正的权限边界是后端的
>    `AdminOnly` / `ContentWriter` 授权策略，以及「归属只认 `CreatedByUserId`」的校验。
>    多一个登录页既拦不住越权，也挡不住伪造请求。
> 3. 「怕混在一起就注册成管理员」这个担心不成立：**本站没有注册端点**（T1），
>    账号一律由管理员在 `/api/users` 创建，与登录页数量无关。
>
> 因此合并为：`POST /api/auth/login`（角色无关）+ 单一页面 `/login`，
> 登录后按响应中的 `role` 跳 `/admin` 或 `/me`；旧路径 `/admin/login` 保留为重定向。
>
> **必须记住的术语区分**：
> - `User`：登录账号（邮箱 + 密码 + 角色），是**认证主体**。
> - `Author`：署名与资料（姓名/头像/简介），是**内容实体**，不是认证主体。
> - `User.AuthorId` 可选关联一个 `Author`；发文时 `Post.AuthorId` 决定**署名**，
>   `Post.CreatedByUserId` 决定**归属与改删权限**，两者不可混用。

#### `[待确认]` 单应用还是双应用

| 方案 | 说明 | 建议 |
|---|---|---|
| **A. 单一 SPA + 角色路由** | 一个 Vue 应用，单一 `/login` 统一登录；路由守卫按 `role` 拦截，登录后按角色落地 | **推荐**：复用现有组件与 API 层，成本最低 |
...
### 3.6 上传图片自动转 WebP（省传输流量）

**目标**：站内所有上传图（文章封面、作者头像、首屏 hero）在**落盘前**统一转 WebP。

| 项 | 决定 | 理由 |
|---|---|---|
| 压缩放在哪一层 | **后端**（`POST /api/files/upload` 内） | 后端 API 是**真正的信任边界**。放前端（canvas 转）虽然零依赖，但脚本/未来 App 一绕就失效 |
| 图像库 | **`SixLabors.ImageSharp` 3.1.12** | 纯托管、无原生依赖，部署最简 |
| **为何不用 4.x** | **v4 起要求构建期许可密钥 `sixlabors.lic`** | 社区许可免费但需先申请，否则**构建直接失败** —— 给构建引入外部审批依赖，不值得 |
| 许可 | Six Labors Split License | 开源 或 年营收 < 100 万美元 → 视同 Apache-2.0。**若将来商业化且营收 ≥ 100 万美元需购买商业许可** |
| 备选（未选） | `SkiaSharp` | Apache-2.0 无营收门槛、无许可密钥；但带原生二进制（每平台十几~几十 MB），Docker 与部署更重 |
| 格式策略 | 只转 `.png/.jpg/.jpeg` → 有损 WebP，质量 82，最长边 2560 | `.webp` 已最优；`.svg` 是矢量不能栅格化；`.gif` 有动画风险 |
| 落盘方式 | **只存 WebP，不保留原图** | 目标是省流量与空间；代价是不可逆（要重压只能重新上传） |

**核心安全约定**：`IImageOptimizer` **永不抛异常、永不导致上传失败**。
多帧图、损坏文件、解码失败、转换后反而更大 —— 一律原样保存并记 WARN。
宁可存一张大图，也不能让用户丢文件。

**实现与测试**见 [backend.md](./backend.md) §8.2.1；`Blog.Tests` 有 18 个用例覆盖上表的每一条边界。
...
| T16 | 登录入口：两个页面还是合并？ | 保持两个（作者/管理员）/ 合并为单一入口 | **合并**。`Author` 是署名实体、无法登录，双页面只造成命名歧义且不提供任何安全性 | §2.3 |
| T17 | 上传图片压缩放在哪层？ | 后端统一 / 前端 canvas / 两者 | **后端统一**（API 才是信任边界）。库选 ImageSharp **3.x** 而非 4.x（v4 需构建期许可密钥） | §3.6 |

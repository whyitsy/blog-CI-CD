...
| **CI 流水线**（GitHub Actions，三个并行作业：行尾检查 / 前端 / 后端 81 项测试），含「防假绿」测试计数断言 | 完成（2026-09-13，首次运行全绿，约 2 分钟） |
...
**工程化与交付能力缺口**（2026-09 识别，编号 G1~G10，详见 [09](./09-已知限制与技术债.md) §6）：

| 项 | 性质 | 状态 |
|---|---|---|
| **流水线已建成，但未开启分支保护** | 交付能力缺口 | 🟡 流水线 ✅ / 分支保护 ❌ |
...
> **目前「应用 Dockerfile」已解决；「CI/CD」的流水线部分已落地，
> 但分支保护尚未开启 —— 也就是说 CI 现在提供「信息」但不提供「约束」。**
>

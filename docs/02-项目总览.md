...
| 项 | 性质 | 详见 |
|---|---|---|
| 开发用 PostgreSQL 容器尚未换成带 zhparser 的自定义镜像 | **风险**：容器重建则搜索失效 | [09](./09-已知限制与技术债.md) §2 |
| 全文检索缺少相关度排序（`ts_rank`） | 功能缺失：当前按发布时间倒序 | [09](./09-已知限制与技术债.md) §3 |
| 测试覆盖率低（仅图片优化器有单元测试） | 技术债 | [09](./09-已知限制与技术债.md) §4 |
| 种子作者头像路径 `/media/avatar-default.png` 已无效 | 小瑕疵：前端已回退到首字母占位 | [09](./09-已知限制与技术债.md) §5 |
| 前端表单"点击保存"链路未做浏览器自动化验证 | 验证缺口（后端语义已用 curl 逐条验证） | [09](./09-已知限制与技术债.md) §6 |

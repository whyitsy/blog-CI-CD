...
| 数据库迁移（2 个迁移，10 张表） | 完成 |
| 前端认证闭环（登录页、路由守卫、401 自动清理、按角色落地） | 完成 |
| 骨架屏组件化（6 个组件） | 完成 |
| 图片上传自动转 WebP | 完成 |
| 测试体系：集成（权限矩阵/草稿保护/乐观锁）+ 单元（Service/图片边界），**81 项** | 完成 |
...
| 项 | 性质 | 详见 |
|---|---|---|
| 前端表单"点击保存"链路未做浏览器自动化验证 | 验证缺口（后端 81 项测试已覆盖语义） | [09](./09-已知限制与技术债.md) §1 |
| `FileStorage:Root` 依赖当前工作目录（T19） | 部署健壮性 | [07](./07-开发与运维手册.md) §8.2 |
| SEO（站点地图、结构化数据） | 明确决定**不做** | — |
| 评论数据自建 | 明确决定**不做**（外挂 giscus） | — |

**工程化与交付能力缺口**（2026-09 识别，共 9 项，编号 G1~G9）：

| 项 | 性质 | 详见 |
|---|---|---|
| **完全没有 CI/CD**（无 `.github/`、无任何 `*.yml`） | 交付能力缺口 | [09](./09-已知限制与技术债.md) §6、[11](./11-CI-CD与自动化交付.md) |
| 前端自动化脚本放在仓库外的 `/mnt/d/tmpbuild/` | 资产丢失风险 | [09](./09-已知限制与技术债.md) §6 |
| 没有应用自身的 Dockerfile（只有 PG 的） | 交付能力缺口 | [11](./11-CI-CD与自动化交付.md) §8 |
| 没有发布流程、版本号与回滚方案 | 交付能力缺口 | [11](./11-CI-CD与自动化交付.md) §8.3 |
| 没有依赖安全扫描；ImageSharp 锁在 3.x | 供应链风险 | [11](./11-CI-CD与自动化交付.md) §9.4 |
| 没有 lint / 格式门禁（无 `.editorconfig`、前端无 ESLint） | 质量门禁缺口 | [11](./11-CI-CD与自动化交付.md) §9.3 |
| 没有覆盖率度量（`coverlet` 已引入但无报告） | 度量缺口 | [11](./11-CI-CD与自动化交付.md) §9.2 |
| 没有性能基线、没有 SLO | 容量未知 | [12](./12-性能测试与容量分析.md) §6 |
| 没有 metrics / tracing（只有文本日志） | 可观测性缺口 | [12](./12-性能测试与容量分析.md) §11.2 |

> ⚠️ **这九项不是"故障隐患"，而是"能力缺口"**——
...
>
> **性能测试目前做不了**：缺少部署形态、缺少 SLO，且开发库只有 9 篇文章
> （索引完全不生效）。原因与正确顺序见 [12](./12-性能测试与容量分析.md) §2。

...
| 骨架屏组件化（6 个组件） | 完成 |
| 图片上传自动转 WebP | 完成 |
| 首个测试工程 `Blog.Tests`（图片优化器边界测试） | 完成 |

### 7.2 未完成 / 待处理

| 项 | 性质 | 详见 |
|---|---|---|
| 全文检索缺少相关度排序（`ts_rank`） | 功能缺失：当前按发布时间倒序 | [09](./09-已知限制与技术债.md) §1 |
| 测试覆盖率低（仅图片优化器有单元测试） | 技术债 | [09](./09-已知限制与技术债.md) §2 |
| 前端表单"点击保存"链路未做浏览器自动化验证 | 验证缺口（后端语义已用 curl 逐条验证） | [09](./09-已知限制与技术债.md) §3 |
| 没有健康检查端点 | 运维缺失（多实例部署前应补） | [09](./09-已知限制与技术债.md) §4 |
| 种子作者头像路径 `/media/avatar-default.png` 已无效 | 小瑕疵：前端已回退到首字母占位 | [09](./09-已知限制与技术债.md) §5 |
...
> 记录见 [09-已知限制与技术债.md](./09-已知限制与技术债.md) §10。
